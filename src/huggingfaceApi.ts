...
export namespace HuggingFaceApi {
    let outputChannel: vscode.OutputChannel = vscode.window.createOutputChannel('TDS-Dito');
    ...
    export async function _getCompletions3(textBeforeCursor: string, textAfterCursor: string): Promise<string | undefined> {
        ...
        //<fim_prefix>beforeCursor<fim_suffix>afterCursor<fim_middle>"
        const body: {} = {
            "inputs": `<fix_prefix>${textBeforeCursor}<fim_suffix>${textAfterCursor}<fim_middle>`,
            "parameters": {
                "top_k": config.top_k,
                "top_p": config.top_p,
                "temperature": config.temperature,
