...
export function registerHealth(context: vscode.ExtensionContext, iaApi: IaApiInterface, chatApi: ChatApi): void {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('tds-dito.health', async (...args) => {
        ...
        if (attempt == 1) {
            messageId = chatApi.dito("Verificando disponibilidade do serviço.");
        }
                ...
                if (error !== undefined) {
                    ...
                    if (error.message.includes("502: Bad Gateway")) {
                        ...
                        if ((attempt <= totalAttempts) && (time !== null)) {
                            ...
                            chatApi.dito([
                                `Desculpe, mesmo após **${totalAttempts} tentativas**, ainda continuo com dificuldades técnicas.`,
                                `Para reiniciar a validação do serviço, acione ${chatApi.commandText("health")}.`],
                                messageId);
                        }
                    } else {
                        chatApi.dito("Disponível!", messageId);
                        vscode.window.showInformationMessage(`${PREFIX_DITO} Disponível!`);
                    }
...
function tryAgain(attempt: number, totalAttempt: number, time: number): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
        vscode.window.withProgress<string>({
            ...
            const cancelPromise = promiseFromEvent<any, any>(token.onCancellationRequested, (_, __, reject) => { reject('User Cancelled'); }).promise;
            const delayPromise = new Promise<string>((resolve, _) => {

                const interval = setInterval(() => {
                    const msg: string = `Verificando disponibilidade em ${time} segundos. (${attempt}/${totalAttempt})`;
